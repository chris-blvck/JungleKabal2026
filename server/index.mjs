...
function createEmptyMiniappState() {
  return {
    version: 1,
    runs: [],
    competitiveDailyRuns: [],
    openPracticeRuns: [],
    competitiveBonusByUser: {},
    referrals: [],
    referralClaims: [],
    statsByCode: {},
    telemetry: [],
...
function miniappDayKey(date = new Date()) {
...
async function getMiniappState() {
  try {
    ...
    return {
      ...createEmptyMiniappState(),
      ...parsed,
      runs: Array.isArray(parsed.runs) ? parsed.runs : [],
      competitiveDailyRuns: Array.isArray(parsed.competitiveDailyRuns) ? parsed.competitiveDailyRuns : [],
      openPracticeRuns: Array.isArray(parsed.openPracticeRuns) ? parsed.openPracticeRuns : [],
      competitiveBonusByUser: parsed.competitiveBonusByUser && typeof parsed.competitiveBonusByUser === 'object' ? parsed.competitiveBonusByUser : {},
    };
  } catch {
...
const server = createServer(async (req, res) => {
  ...
  if (pathName === '/api/runs/finish' && req.method === 'POST') {
    try {
      ...
      const mode = body.mode === 'practice' ? 'practice' : 'competitive';
      const dailyKey = miniappDayKey();
      const competitiveBonusByUser = state.competitiveBonusByUser || {};
      ...
      if (mode === 'competitive') {
        const bonusTickets = Math.max(0, Number(competitiveBonusByUser[auth.userId] || 0));
        const maxAttempts = 1 + bonusTickets;
        const playedToday = state.runs.filter((entry) => entry.telegramUserId === auth.userId && entry.mode === 'competitive' && entry.dailyKey === dailyKey).length;
        if (playedToday >= maxAttempts) {
          return send(res, 403, {
            ok: false,
            error: 'Daily competitive attempts exhausted.',
            limits: { mode: 'competitive', maxAttempts, playedToday, remaining: 0 },
          });
        }
      } else {
        const playedToday = state.runs.filter((entry) => entry.telegramUserId === auth.userId && entry.mode === 'practice' && entry.dailyKey === dailyKey).length;
        const maxAttempts = 10;
        if (playedToday >= maxAttempts) {
          return send(res, 403, {
            ok: false,
            error: 'Daily practice attempts exhausted (10/day).',
            limits: { mode: 'practice', maxAttempts, playedToday, remaining: 0 },
          });
        }
      }
      ...
      if (mode === 'competitive' && Number(body.floor) >= 6) {
        competitiveBonusByUser[auth.userId] = Math.max(0, Number(competitiveBonusByUser[auth.userId] || 0)) + 1;
      }

      state.runs.push(run);
      state.runs = state.runs.sort((a, b) => b.score - a.score).slice(0, 2000);
      if (mode === 'competitive') {
        state.competitiveDailyRuns = [...(state.competitiveDailyRuns || []), run].slice(-5000);
      } else {
        state.openPracticeRuns = [...(state.openPracticeRuns || []), run].slice(-5000);
      }
      state.competitiveBonusByUser = competitiveBonusByUser;
  ...
  if (pathName === '/api/runs/leaderboard' && req.method === 'GET') {
    ...
    if (mode === 'competitive') {
      const today = miniappDayKey();
      const board = (state.competitiveDailyRuns || [])
        .filter((entry) => entry.dailyKey === today)
        .sort((a, b) => b.score - a.score)
        .slice(0, limit);
      return send(res, 200, { ok: true, mode: 'competitive', day: today, leaderboard: board });
    }
    if (mode === 'practice') {
      const board = [...(state.openPracticeRuns || [])].sort((a, b) => b.score - a.score).slice(0, limit);
      return send(res, 200, { ok: true, mode: 'practice', leaderboard: board });
    }
    return send(res, 200, { ok: true, mode: 'all', leaderboard: state.runs.slice(0, limit) });
  }
