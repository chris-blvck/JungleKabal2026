...
async function fetchSolBalance(address) {
  ...
  if (!response.ok) throw new Error(`SOL RPC error (${response.status})`);
...
async function fetchSolUsdPrice() {
  ...
  if (!response.ok) throw new Error(`SOL price error (${response.status})`);
  const data = await response.json();
...
export default function AngelOpsDashboard() {
  ...
  return (
    <Shell
      ...
      subtitle="Mini-app vertical flow · premium visual · trust-first UX"
      maxWidth={980}
    >
