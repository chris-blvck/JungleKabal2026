...
// 90-day goals · March–May 2026 · Persistent via API
// ============================================================
...
const INITIAL_GOALS = [
  { id: 1, category: "FINANCE", title: "Hit $31K combined P&L",      description: "KKM + Kabal + LP combined target for the sprint", target: 31000, current: 4200, unit: "$",       priority: "HIGH", status: "active" },
  { id: 2, category: "TRADING", title: "Maintain 60%+ win rate",      description: "Across all KKM and Kabal trades",                target: 60,    current: 58,   unit: "%",       priority: "HIGH", status: "active" },
  { id: 3, category: "OPS",     title: "Ship 6 working tools",        description: "Watchlist, PNL Calendar, Risk Manager, Sprint Board, Arsenal, CRM", target: 6, current: 4, unit: "tools",   priority: "MED",  status: "active" },
  { id: 4, category: "GROWTH",  title: "Grow squad to 8 members",     description: "3 additional members with proven track records",  target: 8,     current: 5,   unit: "members", priority: "MED",  status: "active" },
  { id: 5, category: "FINANCE", title: "Build 8 weeks ops runway",    description: "Ensure operations covered for 2 months ahead",   target: 8,     current: 5,   unit: "weeks",   priority: "HIGH", status: "active" },
  { id: 6, category: "DEALS",   title: "Close 3 Angel deals",         description: "KKM angel round — min $50K each",                target: 3,     current: 0,   unit: "deals",   priority: "HIGH", status: "active" },
];

const CAT_COLORS = { FINANCE: JK.green, TRADING: "#F5A623", OPS: "#3B82F6", GROWTH: "#A855F7", DEALS: "#EC4899" };
...
function GoalCard({ goal, onUpdate, onDelete }) {
  ...
  function save() { onUpdate(goal.id, parseFloat(val) || 0); setEditing(false); }

  return (
    <div style={{ background: "rgba(18,18,18,0.9)", border: `1px solid ${pct >= 100 ? JK.green + "44" : "rgba(255,255,255,0.06)"}`, borderRadius: 14, padding: "16px 18px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          ...
          <button onClick={() => onDelete(goal.id)} style={{ background: "rgba(239,68,68,0.08)", border: "none", color: JK.red, borderRadius: 6, fontSize: 9, padding: "2px 6px", cursor: "pointer" }}>✕</button>
        </div>
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ fontSize: 10, color: "#555" }}>
          {editing ? (
            ...
            <span onClick={() => setEditing(true)} style={{ cursor: "pointer" }} title="Cliquer pour modifier">
...
export default function SprintBoard() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/sprint/goals`)
      .then(r => r.json())
      .then(data => {
        if (data.ok && Array.isArray(data.goals) && data.goals.length) {
          setGoals(data.goals);
        }
      })
      .catch(() => {})
  ...
  // Auto-save to API 1.5s after any change
  useEffect(() => {
    if (!loaded) return;
    clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(() => persistGoals(goals), 1500);
  ...
  async function persistGoals(nextGoals) {
    setSaving(true);
    try {
      const r = await fetch(`${API_BASE}/api/sprint/goals`, {
        method: "PUT",
  ...
  return (
    <Shell title={<>SPRINT <span style={{ color: JK.gold }}>BOARD</span></>} subtitle="90-Day Goals · March – May 2026 · Epoch 1 · Partagé avec toute l'équipe" maxWidth={960}>
      ...
      <div style={{ marginBottom: 16, display: "flex", justifyContent: "flex-end" }}>
        <button onClick={() => setAddMode(!addMode)} style={{ background: `rgba(245,166,35,0.12)`, border: `1px solid ${G}44`, borderRadius: 8, color: G, fontWeight: 700, fontSize: 11, letterSpacing: 1.5, padding: "9px 18px", cursor: "pointer", fontFamily: "inherit" }}>
          {addMode ? "CANCEL" : "+ ADD GOAL"}
        </button>
      </div>
