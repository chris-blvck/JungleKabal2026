...
type DieInTheJungleProps = {
  ...
  uiPreset?: 'default' | 'readable';
  fxIntensity?: 'low' | 'normal' | 'high';
...
const KREX_AVATAR_URL = "https://i.postimg.cc/J7kfZT90/Chat-GPT-Image-Mar-12-2026-03-00-38-PM.png";
...
const PLAYER_CHARACTERS = {
  ...
  krex: {
    ...
    subtitle: "Berserk · 30 HP · +2 ATK · 1 reroll",
    stats: { maxHp: 30, attackBonus: 2, rerollsPerTurn: 1, combatStartShield: 1, healBonus: 0 },
  },
};

const COMPANION_BUFFS = {
  momo: { id: 'momo', name: 'Momo Monkey', passive: { attackBonus: 1 } },
  kappa: { id: 'kappa', name: 'Kappa Turtle', auto: { every: 3, kind: 'shield', value: 4 } },
  pixi: { id: 'pixi', name: 'Pixi Spirit', auto: { every: 2, kind: 'heal', value: 2 } },
};

const STARTER_WEAPON_BUFFS = {
  'jungle-blade-common': { name: 'Jungle Blade', passive: { attackBonus: 2 } },
  'root-staff-rare': { name: 'Root Staff', passive: { healBonus: 2, maxHp: 2 } },
  'stone-shield-rare': { name: 'Stone Shield', passive: { combatStartShield: 2, maxHp: 3 } },
  'venom-fang-epic': { name: 'Venom Fang', passive: { attackDieValueBonus: 1, attackBonus: 1 } },
};

function getCharacterName(characterId = "kabalian") {
  return PLAYER_CHARACTERS[characterId]?.name || PLAYER_CHARACTERS.kabalian.name;
}
...
function getStoryFragment(fragmentIndex, characterId) {
  ...
  const krex = [
    {
      title: "K-Rex Chronicle — Fragment I · Birth of the Fang",
      lines: [
        "The jungle carved me in rage.",
        "Every hunt sharpened my instincts.",
        "Now I hunt the Ka itself.",
      ],
    },
    {
      title: "K-Rex Chronicle — Fragment II · Trial by Bones",
      lines: [
        "Power has a smell.",
        "Fear has one too.",
        "I follow both.",
    ...
    {
      title: "K-Rex Chronicle — Fragment III · Alpha Signal",
      lines: [
        "Kabalian calls it strategy.",
        "I call it momentum.",
        "Break one line, break them all.",
      ],
    },
    {
      title: "K-Rex Chronicle — Fragment IV · Blood Debt",
      lines: [
        "The jungle takes payment in scars.",
        "I kept moving anyway.",
        "That is why I survived.",
      ],
    },
    {
      title: "K-Rex Chronicle — Fragment V · Last Roar",
      lines: [
        "The crown can command armies.",
        "Or wake ancient monsters.",
        "Either outcome sounds fun.",
      ],
    },
    {
      title: "K-Rex Chronicle — Fragment VI · Apex",
      lines: [
        "The jungle respects only winners.",
        "I am done waiting.",
        "Let the hunt begin.",
      ],
    },
  ...
  const fragmentsByCharacter = {
    kabalian,
    kkm,
    krex,
  };
  const fragments = fragmentsByCharacter[characterId] || kabalian;
...
function applyBuildLoadout(player, selectedStarterWeaponId, selectedCompanionId) {
  let next = { ...player };
  const weapon = STARTER_WEAPON_BUFFS[selectedStarterWeaponId || ''];
  const companion = COMPANION_BUFFS[selectedCompanionId || ''];

  if (weapon?.passive) {
    if (weapon.passive.attackBonus) next.attackBonus += weapon.passive.attackBonus;
    if (weapon.passive.attackDieValueBonus) next.attackDieValueBonus += weapon.passive.attackDieValueBonus;
    if (weapon.passive.healBonus) next.healBonus += weapon.passive.healBonus;
    if (weapon.passive.maxHp) {
      next.maxHp += weapon.passive.maxHp;
      next.hp = Math.min(next.maxHp, next.hp + weapon.passive.maxHp);
    }
    if (weapon.passive.combatStartShield) {
      next.combatStartShield += weapon.passive.combatStartShield;
      next.shield += weapon.passive.combatStartShield;
    }
  }

  if (companion?.passive?.attackBonus) next.attackBonus += companion.passive.attackBonus;
  return next;
}
...
export default function DieInTheJungleUpgraded({
  ...
  uiPreset = 'default',
  fxIntensity = 'normal',
  unlockedCharacterIds = ['kabalian', 'kkm'],
  ...
  function shareRun() {
    const characterName = getCharacterName(game.player.characterId);
  ...
  function pickCharacter(characterId) {
    const selected = PLAYER_CHARACTERS[characterId] || PLAYER_CHARACTERS.kabalian;
    if (!unlockedCharacterIds.includes(selected.id)) {
      setGame((g) => ({ ...g, actionFlash: { id: Date.now(), text: `🔒 ${selected.name} is locked`, tone: "rose" } }));
      return;
    }
    setGame((g) => {
      const nextPlayer = applyBuildLoadout({
        ...g.player,
        characterId: selected.id,
        avatar: selected.avatar,
        maxHp: selected.stats.maxHp,
        ...
        rerollsPerTurn: selected.stats.rerollsPerTurn,
        rerollsLeft: selected.stats.rerollsPerTurn,
      }, selectedStarterWeaponId, selectedCompanionId);
      return {
        ...g,
        player: nextPlayer,
  ...
  function startRoll() {
    ...
    setGame((g) => {
      ...
      const companion = COMPANION_BUFFS[selectedCompanionId || ''];
      let lines = [];
      if (companion?.auto && nextTurn % companion.auto.every === 0) {
        if (companion.auto.kind === 'shield') {
          player.shield += companion.auto.value;
          lines.push(`🐾 ${companion.name}: +${companion.auto.value} shield auto`);
        } else if (companion.auto.kind === 'heal') {
          player.hp = Math.min(player.maxHp, player.hp + companion.auto.value);
          lines.push(`🐾 ${companion.name}: +${companion.auto.value} heal auto`);
        }
      }
    ...
    window.setTimeout(() => {
      setGame((g) => {
        ...
        return {
          ...
          actionFlash: {
            id: Date.now(),
            text: `${companion?.auto && nextTurn % companion.auto.every === 0 ? '🐾 Companion proc · ' : ''}🎲 ${dice.map((d) => `${getDieMeta(d).emoji}${d.value}`).join(" · ")}`,
            tone: "amber",
          },
  ...
  const avatarUrl = PLAYER_EMOTION_URLS[game.avatarMood] || game.player.avatar || PLAYER_AVATAR_URL;
  const isReadable = uiPreset === 'readable';
  const fxSpawnCount = fxIntensity === 'low' ? 6 : fxIntensity === 'high' ? 18 : 14;
  const fxDuration = fxIntensity === 'low' ? 0.8 : fxIntensity === 'high' ? 1.6 : 1.3;
  ...
  return (
    <div
      className={`min-h-screen overflow-y-auto bg-cover bg-center bg-no-repeat p-2 text-white ${isReadable ? 'text-[15px]' : ''}`}
      style={{ backgroundImage: `linear-gradient(${isReadable ? 'rgba(0,0,0,.75), rgba(0,0,0,.9)' : 'rgba(0,0,0,.62), rgba(0,0,0,.78)'}), url(${BG_URL})` }}
    >
      <div className="mx-auto flex min-h-screen max-w-6xl flex-col gap-1.5 pb-3 md:gap-2">
            ...
            <div className="rounded-[18px] border border-cyan-300/30 bg-gradient-to-b from-cyan-950/40 to-black/85 p-2">
              <div className="grid grid-cols-2 gap-1.5 rounded-[14px] border border-cyan-300/30 bg-black/35 p-2">
                ...
                <div className="min-w-0">
                  <div className="font-black">{getCharacterName(game.player.characterId)}</div>
        ...
        <AnimatePresence>
          {game.characterSelectPending ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm">
              <div className="w-full max-w-3xl rounded-[28px] border border-cyan-300/25 bg-zinc-950/95 p-5 shadow-[0_20px_80px_rgba(0,0,0,0.55)]">
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  {Object.values(PLAYER_CHARACTERS).map((character) => {
                    const locked = !unlockedCharacterIds.includes(character.id);
                    return (
                    <button
                      key={character.id}
                      onClick={() => !locked && pickCharacter(character.id)}
                      className={`rounded-2xl border border-white/15 bg-black/45 p-3 text-left transition ${locked ? 'cursor-not-allowed opacity-60' : 'hover:border-amber-300/60 hover:bg-black/70'}`}
                      disabled={locked}
                    >
                      ...
                      {locked ? <div className="mt-1 text-[11px] font-black text-rose-300">🔒 Locked (unlock in Build Prep)</div> : null}
                    </button>
                  );
                  })}
                </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm">
              <div className="w-full max-w-xl rounded-[28px] border border-amber-300/25 bg-zinc-950/95 p-5 shadow-[0_20px_80px_rgba(0,0,0,0.55)]">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div className="rounded-xl border border-white/10 bg-black/35 px-3 py-2">Character: <span className="font-black">{getCharacterName(game.player.characterId)}</span></div>
